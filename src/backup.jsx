...
export default function App() {
  ...
  // --- C. SPILLER-BEVÆGELSE (Den avancerede del) ---
  // Denne beregner hvert millisekund præcis hvor på en togskinne du befinder dig

  // --- SPILLER-MARKØRER (Med indbygget dekoder-fallback) ---
  useEffect(() => {
    ...
    gameState.players.forEach(p => {
      ...
      if (p.isTraveling && p.segments?.[0]) {
        const seg = p.segments[0];
        const prog = Math.max(0, Math.min(1, (interpolatedTime - seg.departure) / (seg.arrival - seg.departure)));

        const rawPath = seg.path;
        let finalPath = null;

        if (typeof rawPath === 'string') {
          // Hvis biblioteket mangler i index.html, bruger vi denne lille funktion til at dekode
          ...
          try {
            ...
            console.error("Dekodning fejlede", e);
          }
        ...
        if (finalPath && finalPath.length > 1) {
          ...
          const s = CITIES[seg.from]?.pos || pos;
          const e = CITIES[seg.to]?.pos || pos;
      ...
      // Her henter vi spillerens farve, eller bruger en standard blå hvis den mangler
    ...
    if (markersRef.current[p.id]) {
      ...
      markersRef.current[p.id].setIcon(window.L.divIcon({
        html: iconHtml,
        className: '',
        iconSize: [30, 30],
        iconAnchor: [15, 15]
  ...
  const travel = async (dep) => {
    ...
    // Sikkerhedstjek: Er spilleren allerede undervejs, eller har de ikke råd?
    if (!me || me.isTraveling || me.money < dep.cost) return;

    // --- 1. OPTIMISTIC UI UPDATE ---
    // Vi opdaterer den lokale tilstand med det samme, så skærmen skifter
    // lynhurtigt uden at vente på svar fra Firebase.
    ...
    setGameState(prev => ({
      ...prev,
      players: prev.players.map(p => p.id === user.uid ? optimisticPlayer : p)
    }));

    // --- 2. BEREGN REJSENS SEGMENTER ---
    const segments = [];
    let startTime = dep.time;

    // Vi løber igennem ruten (dep.route) som blev genereret i departures useMemo
    for (let i = 0; i < dep.route.length - 1; i++) {
      const from = dep.route[i];
      const to = dep.route[i + 1];

      const pos1 = CITIES[from]?.pos;
      const pos2 = CITIES[to]?.pos;

      if (!pos1 || !pos2) continue;

      const dist = getDist(pos1, pos2);

      // Find hastighed baseret på transporttype (tog, bus, fly, færge)
      const config = TRAVEL_TYPES[dep.type] || TRAVEL_TYPES.bus;
      const duration = (dist / config.speed) * 60; // Tid i minutter
      ...
      // Læg rejsetid + stop-varighed (f.eks. 12 min) til, før næste segment starter
      startTime += duration + STOP_DURATION;
    }

    // --- 3. OPDATER DATABASE (Firebase) ---
    try {
      ...
      p.id === user.uid ? {
        ...
        travelType: dep.type,
        currentCity: me.currentCity // Vi beholder startbyen indtil ankomst-loopet i hosten flytter os
      } : p
      );

      await updateDoc(playerRef, {
        players: updatedPlayers,
        // Vi tilføjer en log-besked så andre kan se rejsen
        logs: [{
          time: gameState.gameTime,
          message: `${me.name} er rejst fra ${me.currentCity} mod ${dep.destination}`,
          type: 'info'
        }, ...(gameState.logs || [])].slice(0, 50)
      });
    } catch (error) {
      console.error("Fejl under rejse-opdatering:", error);
      // Ved fejl: Genindlæs gameState fra databasen for at rulle tilbage
      listen(gameState.code);
    }
  };
